...
void BaseEye::update(LGFX_Sprite *canvas, BoundingRect rect, DrawContext *ctx) {
...
void EllipseEye::draw(LGFX_Sprite *canvas, BoundingRect rect, DrawContext *ctx) {
...
void GirlyEye::drawEyeLid(LGFX_Sprite *canvas) {
    // eyelid
...
void GirlyEye::draw(LGFX_Sprite *canvas, BoundingRect rect, DrawContext *ctx) {
    this->update(canvas, rect, ctx);
    ...
    if (open_ratio_ > 0.1f) {
        ...
        uint16_t accent_color = Display.color24to16(0x019E73);
...
void PinkDemonEye::drawEyeLid(LGFX_Sprite *canvas) {
    // eyelid
    auto upper_eyelid_y = shifted_y_ - 0.8f * height_ / 2 +
                          (1.0f - open_ratio_) * this->height_ * 0.6;
...
void PinkDemonEye::draw(LGFX_Sprite *canvas, BoundingRect rect, DrawContext *ctx) {
    this->update(canvas, rect, ctx);
    this->overwriteOpenRatio();
    ...
    if (open_ratio_ > 0.1f) {
        ...
        canvas->fillEllipse(shifted_x_, shifted_y_, this->width_ / 2,
                            this->height_ / 2, primary_color_);
        uint16_t accent_color = Display.color24to16(0x00A1FF);
        canvas->fillEllipse(shifted_x_, shifted_y_,
                            this->width_ / 2 - thickness,
                            this->height_ / 2 - thickness, accent_color);
        // upper
...
void DoggyEye::draw(LGFX_Sprite *canvas, BoundingRect rect, DrawContext *ctx) {
    this->update(canvas, rect, ctx);
