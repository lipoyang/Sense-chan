...
void fillRotatedRect(LGFX_Sprite *canvas, uint16_t cx, uint16_t cy, uint16_t w,
                     uint16_t h, float angle, uint16_t color);

void fillRectRotatedAround(LGFX_Sprite *canvas, float top_left_x, float top_left_y,
